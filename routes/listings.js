...
const validateCategoryId = (req, res, next) => {
  ...
  if (!categoriesStore.getCategory(parseInt(req.body.categoryId)))
    return res.status(400).send({ error: "No categoryId." });
...
router.post(
  ...
  [
    ...
    // auth,
    upload.array("image", config.get("maxImageCount")),
    validateCategoryId,
    imageResize,
  ],
  async (req, res) => {
    const { title, description, price, categoryId } = req.body;
    const result = schema.validate({ title, description, price, categoryId });
    if (result.error)
